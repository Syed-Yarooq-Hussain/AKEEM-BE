...
export class Employee extends BaseModel<Employee> {
  ...
  @ForeignKey(() => Employee)
  @Column(DataType.INTEGER)
  declare managerEmployeeId?: number;
  @Column({ allowNull: false }) declare employeeNumber: string;
  ...
  @Column declare employmentType?: string;
  @Column declare hireDate?: Date;
  @Column declare terminationDate?: Date;
  @Column({ defaultValue: 'active' }) declare status: string;
  @Column({ type: DataType.DECIMAL(14, 2) }) declare salary?: number;
  @Column declare salaryCurrency?: string;
...
export class LeaveRequest extends BaseModel<LeaveRequest> {
  ...
  @Column declare endDate: Date;
  @Column({ type: DataType.DECIMAL(5, 2) }) declare days: number;
  @Column({ defaultValue: 'pending' }) declare status: string;
  @Column({ type: DataType.TEXT }) declare reason?: string;
  @Column({ type: DataType.TEXT }) declare reviewNote?: string;
}
...
export class AttendanceEntry extends BaseModel<AttendanceEntry> {
  ...
  @Column(DataType.INTEGER)
  declare employeeId: number;
  @Column declare clockIn: Date;
  @Column declare clockOut?: Date;
  @Column({ type: DataType.DECIMAL(6, 2) }) declare hours?: number;
  @Column({ defaultValue: 'present' }) declare status: string;
