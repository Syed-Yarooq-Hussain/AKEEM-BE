...
export class Project extends BaseModel<Project> {
  ...
  @Column({ allowNull: false }) declare name: string; @Column({ type: DataType.TEXT }) declare description?: string;
  @Column({ defaultValue: 'planned' }) declare status: string; @Column declare color?: string;
  @Column declare startDate?: Date; @Column declare dueDate?: Date;
...
export class Task extends BaseModel<Task> {
  ...
  @ForeignKey(() => Task) @Column(DataType.INTEGER) declare parentTaskId?: number;
  @ForeignKey(() => User) @Column(DataType.INTEGER) declare createdById: number;
  @Column({ allowNull: false }) declare title: string; @Column({ type: DataType.TEXT }) declare description?: string;
  @Column({ defaultValue: 'todo' }) declare status: string; @Column({ defaultValue: 'medium' }) declare priority: string;
  @Column declare startAt?: Date; @Column declare dueAt?: Date; @Column declare completedAt?: Date;
...
@Table({ tableName: 'task_assignees', underscored: true, timestamps: true, paranoid: false })
...
export class TaskComment extends BaseModel<TaskComment> {
  @ForeignKey(() => Task) @Column(DataType.INTEGER) declare taskId: number;
  @ForeignKey(() => User) @Column(DataType.INTEGER) declare userId: number;
  @ForeignKey(() => TaskComment) @Column(DataType.INTEGER) declare parentId?: number;
  @Column({ type: DataType.TEXT, allowNull: false }) declare body: string;
  @Column({ type: DataType.JSONB, defaultValue: [] }) declare mentions: string[];
}
