import {
  BelongsTo, BelongsToMany, Column, DataType, ForeignKey, HasMany, Table,
...
export class User extends BaseModel<User> {
  ...
  @Column({ defaultValue: 'active' }) declare status: string;
  @Column({ type: DataType.JSONB, defaultValue: {} }) declare preferences: object;
  @HasMany(() => Membership) declare memberships: Membership[];
}

@Table({ tableName: 'memberships', underscored: true, paranoid: true, indexes: [{ unique: true, fields: ['organization_id', 'user_id'] }] })
...
export class Role extends BaseModel<Role> {
  ...
  @BelongsToMany(() => Permission, () => RolePermission) declare permissions: Permission[];
}

@Table({ tableName: 'permissions', underscored: true, timestamps: true, paranoid: false })
...
@Table({ tableName: 'role_permissions', underscored: true, timestamps: true, paranoid: false })
export class RolePermission extends BaseModel<RolePermission> {
  @ForeignKey(() => Role) @Column(DataType.INTEGER) declare roleId: number;
  @ForeignKey(() => Permission) @Column(DataType.INTEGER) declare permissionId: number;
}
...
export class Plan extends BaseModel<Plan> {
  ...
  @Column({ allowNull: false }) declare name: string;
  @Column({ type: DataType.DECIMAL(12, 2), defaultValue: 0 }) declare monthlyPrice: number;
  @Column({ type: DataType.DECIMAL(12, 2), defaultValue: 0 }) declare yearlyPrice: number;
  @Column({ type: DataType.JSONB, defaultValue: {} }) declare limits: object;
  @Column({ type: DataType.JSONB, defaultValue: [] }) declare features: string[];
