...
export class AiAgent extends BaseModel<AiAgent> {
  ...
  @Column({ type: DataType.TEXT }) declare systemPrompt?: string;
  @Column declare model?: string;
  @Column({ type: DataType.DECIMAL(3, 2), defaultValue: 0.7 })
  declare temperature: number;
  @Column({ type: DataType.JSONB, defaultValue: [] }) declare tools: object[];
...
export class AiConversation extends BaseModel<AiConversation> {
  ...
  @ForeignKey(() => AiAgent)
  @Column(DataType.INTEGER)
  declare initiatedByAgentId?: number;
  @Column declare title?: string;
  @Column({ defaultValue: 'active' }) declare status: string;
  @Column({ type: DataType.JSONB, defaultValue: {} }) declare context: object;
  @Column declare lastMessageAt?: Date;
}
...
export class AiMessage extends BaseModel<AiMessage> {
  ...
  @ForeignKey(() => AiAgent)
  @Column(DataType.INTEGER)
  declare senderAgentId?: number;
  @ForeignKey(() => AiAgent)
  @Column(DataType.INTEGER)
  declare recipientAgentId?: number;
  @Column({ allowNull: false }) declare role: string;
  @Column({ type: DataType.TEXT, allowNull: false }) declare content: string;
  @Column declare model?: string;
  @Column({ defaultValue: 0 }) declare inputTokens: number;
  @Column({ defaultValue: 0 }) declare outputTokens: number;
  @Column({ type: DataType.JSONB, defaultValue: [] })
  declare toolCalls: object[];
...
export class KnowledgeDocument extends BaseModel<KnowledgeDocument> {
  ...
  @Column declare sourceType?: string;
  @Column declare sourceUrl?: string;
  ...
  @Column({ type: DataType.JSONB, defaultValue: {} }) declare metadata: object;
  @Column(DataType.INTEGER) declare fileId?: number;
  @Column(DataType.INTEGER) declare projectId?: number;
}

@Table({ tableName: 'knowledge_chunks', underscored: true, paranoid: true })
export class KnowledgeChunk extends BaseModel<KnowledgeChunk> {
  @ForeignKey(() => Organization)
  @Column(DataType.INTEGER)
  declare organizationId: number;
  @Column(DataType.INTEGER) declare projectId?: number;
  @Column(DataType.INTEGER) declare fileId: number;
  @Column(DataType.INTEGER) declare knowledgeDocumentId: number;
  @Column({ allowNull: false }) declare chunkIndex: number;
  @Column({ type: DataType.TEXT, allowNull: false }) declare content: string;
  @Column({ allowNull: false }) declare reference: string;
  @Column({ type: DataType.JSONB, defaultValue: {} }) declare metadata: object;
}
...
export class Automation extends BaseModel<Automation> {
  ...
  @Column({ allowNull: false }) declare name: string;
  @Column declare description?: string;
  @Column({ type: DataType.JSONB, allowNull: false }) declare trigger: object;
  @Column({ type: DataType.JSONB, defaultValue: [] }) declare actions: object[];
  @Column({ defaultValue: false }) declare isActive: boolean;
  @Column declare lastRunAt?: Date;
  @Column declare nextRunAt?: Date;
  @Column({ defaultValue: 3 }) declare retryLimit: number;
  @Column({ defaultValue: 60 }) declare timeoutSeconds: number;
}
...
export class AutomationRun extends BaseModel<AutomationRun> {
  ...
  @Column declare finishedAt?: Date;
  @Column({ defaultValue: 0 }) declare attemptCount: number;
  @Column({ defaultValue: 3 }) declare maxAttempts: number;
  @Column declare availableAt?: Date;
  @Column declare lockedAt?: Date;
  @Column declare idempotencyKey?: string;
  @Column declare triggerType?: string;
  @Column({ type: DataType.TEXT }) declare failureReason?: string;
  @Column({ type: DataType.JSONB, defaultValue: [] })
  declare actionResults: object[];
}
...
export class Integration extends BaseModel<Integration> {
  ...
  @Column(DataType.INTEGER)
  declare organizationId: number;
  @Column({ allowNull: false }) declare provider: string;
  @Column declare displayName?: string;
  @Column({ defaultValue: 'connected' }) declare status: string;
  @Column({ type: DataType.JSONB, defaultValue: {} })
  declare credentialsEncrypted: object;
  @Column({ type: DataType.JSONB, defaultValue: {} }) declare settings: object;
  @Column declare lastSyncedAt?: Date;
}
...
export class AiAgentTeam extends BaseModel<AiAgentTeam> {
  ...
  @Column({ defaultValue: 'active' }) declare status: string;
  @Column({ type: DataType.JSONB, defaultValue: {} })
  declare orchestrationConfig: object;
}

@Table({
  tableName: 'ai_agent_team_members',
  underscored: true,
  timestamps: true,
  paranoid: false,
})
export class AiAgentTeamMember extends BaseModel<AiAgentTeamMember> {
  @ForeignKey(() => AiAgentTeam)
  @Column(DataType.INTEGER)
...
export class AiAgentDelegation extends BaseModel<AiAgentDelegation> {
  ...
  @ForeignKey(() => AiAgent)
  @Column(DataType.INTEGER)
  declare fromAgentId: number;
  @ForeignKey(() => AiAgent)
  @Column(DataType.INTEGER)
  declare toAgentId: number;
  @ForeignKey(() => AiAgentDelegation)
  @Column(DataType.INTEGER)
  declare parentDelegationId?: number;
...
export class Approval extends BaseModel<Approval> {
  ...
  @ForeignKey(() => AiAgent)
  @Column(DataType.INTEGER)
  declare requestedByAgentId?: number;
  @ForeignKey(() => User)
  @Column(DataType.INTEGER)
