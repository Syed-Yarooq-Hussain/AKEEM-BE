...
export class Company extends BaseModel<Company> {
  ...
  @Column declare domain?: string; @Column declare industry?: string;
  @Column declare phone?: string; @Column declare website?: string;
  @Column({ type: DataType.JSONB, defaultValue: {} }) declare address: object;
  @Column({ type: DataType.JSONB, defaultValue: {} }) declare customFields: object;
  @Column({ type: DataType.ARRAY(DataType.STRING), defaultValue: [] }) declare tags: string[];
...
export class Contact extends BaseModel<Contact> {
  ...
  @Column({ allowNull: false }) declare firstName: string; @Column declare lastName?: string;
  @Column declare email?: string; @Column declare phone?: string; @Column declare jobTitle?: string;
...
export class PipelineStage extends BaseModel<PipelineStage> {
  @ForeignKey(() => Pipeline) @Column(DataType.INTEGER) declare pipelineId: number;
  @Column({ allowNull: false }) declare name: string;
...
export class Deal extends BaseModel<Deal> {
  @ForeignKey(() => Organization) @Column(DataType.INTEGER) declare organizationId: number;
  @ForeignKey(() => Pipeline) @Column(DataType.INTEGER) declare pipelineId: number;
  @ForeignKey(() => PipelineStage) @Column(DataType.INTEGER) declare stageId: number;
  @ForeignKey(() => Company) @Column(DataType.INTEGER) declare companyId?: number;
  @ForeignKey(() => Contact) @Column(DataType.INTEGER) declare contactId?: number;
  @ForeignKey(() => User) @Column(DataType.INTEGER) declare ownerId?: number;
  @Column({ allowNull: false }) declare title: string;
  @Column({ type: DataType.DECIMAL(14, 2), defaultValue: 0 }) declare value: number;
...
export class CrmActivity extends BaseModel<CrmActivity> {
  ...
  @Column({ allowNull: false }) declare type: string; @Column declare subject?: string;
  @Column({ type: DataType.TEXT }) declare body?: string; @Column declare occurredAt: Date;
